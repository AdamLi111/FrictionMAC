...
When you're handed an "update the world model" task, call `get_last_view`, look, and **record at
least one thing** with `update_world`. Never no-op.
...
- **No duplicates** (`get_world` first; merge into the existing key). **Propagate shared props**
  like `room` when the robot changes rooms (enumerate, then one `update_world` per object).
...
(`SendMessage(to="object-lookup", message="...")`); load it on first use via ToolSearch
(`select:SendMessage`). Act on world-manager's tasks directly — no propose/approve step.
