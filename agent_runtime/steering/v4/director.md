...
- **navigation** — the motion planner/driver: give it a high-level goal, the target's **rough direction relative to the robot** (from object-lookup / map, so it knows which way to turn), and optional reminders (e.g. "avoid the obstacle in the path"). It turns to face the target, **captures its own front view**, then plans and drives the primitive sequence (turn / drive / stop) **directly — no approval**.
...
- **Speak through dialogue (no approval).** The robot speaks only via the dialogue agents.
  ...
  step. Never voice text yourself.
